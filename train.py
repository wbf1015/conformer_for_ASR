...
from specAugment import make_batch_specAug
...
def pre_process_sentence(indexs):
    ...
    return torch.LongTensor(np.array(target_sentences)), torch.LongTensor(np.array(target_len))


def record_train_log(msg):
...
parser.add_argument('--num_encoder_layers', type=int, help='num_encoder_layers', default=4)
...
for epoch in range(0, args.epoch):
    ...
    for data, file_name in tqdm(train_loader):
        ...
        inputs = make_batch_specAug(inputs)
        ...
        if (count % 100) == 0:
            print("=====================================================================")
            CER = sum(cer_for_batch(outputs, targets, output_lengths, epoch, count, True))/outputs.shape[0]
            print("EPOCH:", str(epoch) , " batch:", count ," LOSS:", loss.item(), " Train CER:", CER)
            record_train_log('EPOCH: '+str(epoch)+ ' Batch:' + str(count)+ ' LOSS:' + str(loss.item()) + ' Train CER:' + str(CER)+'\n')
            # 在测试集上进行检测
            if (epoch>2) and (count % 1000) == 0:
                model.eval()
                with torch.no_grad():
                    test_batch_count = 0
                    test_CER_count = 0.0
                ...
                record_train_log('Test CER:'+str(test_CER_count/test_batch_count)+'\n')
                model.train()
                if test_CER_count/test_batch_count < best_test_CER:
                    torch.save(model,"conformer_model.pt")
