...
def deal_batch_data(batch_data):
    SA = SpecAugment()
    ...
    for index in range(0,batch):
        ...
        new_data.append(SA.run(data))
    
    return torch.tensor(new_data)


if __name__ == "__main__":
    fbank = np.load('/root/autodl-tmp/AIshell1_train/BAC009S0002W0122.npy', allow_pickle=True)
    # fbank_fea = np.random.rand(2048,80)
    # fbank_fea = np.array(fbank_fea, dtype=np.float64)
    # print(fbank_fea)
